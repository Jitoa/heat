...
初期ステップ（fractional-step 法の最初の半ステップ）を行うために必要な作業を行う．<br>
AB2+CN(半陰) 法を動かすための “初期 RHS (右辺）(N^0)” と “固定係数(演算子)” を一括生成し、以後の反復 (FS1→AB2組立→FS2) を成立させる初期化ルーチン<br>
DR(J) : v 点（face 位置）どうしの間隔 Δy_f（face-to-face spacing）<br>
つまり v の 2 階微分や、face 位置で要る中心差分係数に使う<br>
DRR(J) : セル中心（u,w,p 位置）どうしの間隔 Δy_c（center-to-center spacing）<br>
u,w,p の 2 階微分や中心位置での差分係数に使う。

## FS1
時間積分サイクル: (初期化) ZERO → FS0 → (最初の一次元 TDMA) CNTDMAT/CNTDMA → 以後各ステップで FS1 → CNTDMA → FS2（圧力補正・発散修正・統計更新など）<br>
FS0 は「最初のステップ専用」の右辺 N(u)^0 を作り、Adams–Bashforth 2 (AB2) の履歴 CN?_OLD を初期化<br>
FS1 は 2 ステップ目以降で AB2 + y 方向 Crank–Nicolson (CN) 用の「半陰解法用 RHS（仮速度方程式右辺）」を毎ステップ再構築する部分<br>
対応する連続方程式（無次元化後の典型形）<br>
∂u/∂t = - (u·∇)u + (1/Re)(∂²u/∂x² + ∂²u/∂z²) + (1/Re)∂²u/∂y² + F_x - ∂p/∂x<br>
ここで (1/Re)∂²/∂y² のみ CN（半陰）、他（対流 + x,z 拡散 + 外力）は AB2 で陽解法。F_x が一定圧力勾配駆動（コード中の -2RE2.0 の定数項に相当）<br>

## UBC(DIV)
射影法ステップ（FS2 で Poisson→速度補正）後，または途中で呼ばれ、速度場の周期／壁境界をゴーストセルに反映し、離散発散（∇·u）の最大値を計測して非圧縮性保持を監視するユーティリティ．<br>
戻り値 DIV は全セルでの離散発散の最大絶対値（ここでは正値として保持：max|div| ではなく計算式そのものの最大値だが通常ほぼ ≈ |…| に対応）で、数値安定・ソルバ収束の診断指標．<br>
前半 (X-PERIODIC / Z-PERIODIC / Y-NONSLIP): 4 次精度差分で必要なゴーストセルを周期／反射で埋め，壁ノースリップ (w=0) を奇反射 enforcing<br>
後半 (発散計測): 射影法後の非圧縮性維持を数値的に検証。スキュー対称型 4 次差分でエネルギー保存性と低数値発散を確保<br>
戻る DIV は時間積分中の収束監視・デバッグ（例えば発散が閾値以上なら Poisson ソルバ再検討）に利用可能

##  ERG_1STP 
温度 2 成分の y 方向半陰解法用係数と初回 RHS を構築し，AB2 時間積分の履歴配列を初期化して 2 ステップ目以降の ERG に滑らかに接続する “初回専用温度更新カーネル”<br>
ERG_1STP は「温度（または 2 種類のスカラ）T1, T2 の時間積分の“最初の 1 ステップ目”用に、x,z 方向の（明示的扱い）対流・拡散項を組み立て，y 方向の半陰的 Crank–Nicolson 解法（後で CNTDMAT が解く三重対角系）の右辺（FFS1, FFS2）を構築し，次ステップ以降の 2 段 Adams–Bashforth (AB2) に必要な旧 RHS（CNS*_OLD）を初期化するルーチン<br>
つまり “初回だけ AB1（Euler 相当）で種まきし、それ以降 ERG で AB2 を使えるようにする準備”。
...
## CNTDMA 
毎ステップ，前段で組み立てられた速度 RHS (FFX,FFY,FFZ) を y 方向半陰 (三重対角) 解法で解いて暫定速度場 UT を得る fractional-step 中間ステージで，続く Poisson 解 (FS2) の入力を提供する<br>
FS1 までで得た FFX, FFY, FFZ は「y 以外を陽 (AB2) + y 拡散の旧側半分」を含む仮速度方程式の右辺。CNTDMA は y 方向 Crank–Nicolson (半陰) で残り半分の粘性項を組み込む 1 次元（三重対角）解を各 (i,k) ごとに解き，中間速度 UT (圧力未補正) を生成するステップ
...
## BUDGEK1 
第1温度スカラーの二次統計（温度分散 T'² と 3 成分の乱流熱流束 u'θ, v'θ, w'θ）の収支式に出現する全主要項（生成・輸送・分子拡散・散逸・圧力相互作用）を離散形で評価し，J ごとの累積配列へ加算するサンプリングルーチン<br>
壁面チャネル流中の（1）温度（スカラ）変動分散（θ′² の 1/2 あるいはそのまま）および（2）乱流熱流束各成分 ⟨u′θ′⟩, ⟨v′θ′⟩, ⟨w′θ′⟩ の輸送方程式（予算, budget）を、瞬時場から平面平均 y 方向プロファイルとしてサンプリング・蓄積する．結果は後で時間平均され，統計出力（PRO, DIS, TDF, RMDF, PTG, PDF など）として解析される

## BUDGEK2
第2スカラー（TG / TEM2 に格納されるパッシブスカラー）についての乱流温度分散 (T'^2/2) と 3 成分の乱流熱フラックス (u'θ, v'θ, w'θ) の各収支項（生成・拡散・散逸・圧力相関など）を壁法線方向 y ごとに蓄積

## COEF_DEM(PR1,PR2,PPR1,PPR2,PPE1,PPE2,COE1BD,COE2BD)
非圧縮チャネル乱流 DNS における 2 つのスカラー（T1, T2）の時間積分・拡散計算・壁統計処理で必要となる “基本係数” を一括初期化するルーチン<br>
具体的には:<br>

1.時間ステップ・レイノルズ数・プラントル数関連の逆数や拡散係数（PDT, PRE, PPE1, PPE2, PPR1, PPR2, PXZ）を設定<br>
2.壁単位座標 y+ 配列（YPLS1, YPLS2）を構築（YPLS?G との EQUIVALENCE 経由で全域共有）<br>
3.y 方向非一様格子での 1 次／2 次中心差分係数行列（CEN1_*, CEN2_*）を構築<br>
4.スカラー境界条件（壁付近の 2 点離散化）や散逸評価等で用いる係数 COE1BD, COE2BD を計算<br>
これにより後続の ERG_1STP / ERG（スカラー時間発展 RHS 構築）、CNTDMAT（Crank–Nicolson y 方向三重対角解法）、BUDGEK1/BUDGEK2（スカラー分散・熱流束予算）の計算が，連続的・一貫した係数を参照できる
...
## AVE 
500 ステップ毎に瞬時平面統計を評価し，摩擦速度・Reynolds 応力から 4次モーメント・象限・速度勾配不変量までを総和形式で蓄積する“速度場統計のハブ”<br>
1.境界条件を統計計算用に再外挿 (v=0, w 反対称, p 対称)<br>
2.一次平均（AU…）を平面で集計<br>
3.平面平均後，変動成分 (U') を生成<br>
4.二階（Reynolds 応力），三階（AUTRI），四階（AUQUAD）モーメントを逐次加算<br>
5.四分象限解析 (AQAUV / NQAUV) のカウンタ更新<br>
6.勾配テンソル不変量 (AINV2, AINV3) を算出<br>
7.それらを時間積算（AAU 系, AAINV 系など）に追加<br>
8.出力時には平面・時間正規化を別処理(PAVE/DOOP 利用)で行う<br>

## BUDGET 
500 ステップごとの瞬時速度変動場を基に，乱流エネルギーおよび Reynolds 応力方程式の全主要収支項 (生成・散逸・乱流輸送・粘性拡散・圧力拡散・圧力歪・速度‐圧力勾配) を壁法線方向プロファイルとして評価し，時間総和へ蓄積するサブルーチン．正規化は行わず，後段出力で平均化される．UT は直前の AVE が与え，BUDGET の結果は他出力 (OUTPUT) の “BUDGET (K,UU,VV,WW,UV)” 項目に反映される<br>
レイノルズ応力輸送方程式の各項（K, Production, Turbulent Transport, Pressure diffusion, Viscous diffusion, Dissipation, Pressure–Strain 等）を y 方向プロファイルとして瞬時に算出し，それを時間方向に積算するための「源データ」を集計する<br>

流れ:<br>
(a) 必要な瞬時変動速度場 u',v',w'（UT）と平均場 AU を読み込む<br>
(b) 各 j 位置で平面和: 二乗 (u'^2 等), 生産項 Pij, 三重相関からの輸送項 Tajk の差分を計算<br>
(c) （抜粋外）圧力・散逸・再配分など追加計算<br>
(d) 平面和を (IG*KG) で割って平面平均化<br>
(e) AAK 等の A-prefix 配列に時間方向加算（サンプルカウンタ NAVE などでの後の除算）<br>
(f) 所定ステップでファイル出力（典型: Budget_xxxxxx.dat）<br>
